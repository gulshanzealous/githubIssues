...
var publicPath = __dirname.split('/build')[0] + '/frontEnd/public';
app.use(express.static(publicPath));

__webpack_require__(3)(express, app);
...
var serverRenderedView = __dirname.split('/build')[0] + '/frontEnd/public/index.html';
...
// use streams
router.get('/:owner/:repository', function (req, res) {
    console.log('new data');
    var reqUri = baseUri + '/repos/' + req.params.owner + '/' + req.params.repository + '/issues' + '?per_page=50';
    console.log(reqUri);
    ...
    var reponseStream = requestStream.flatMap(function (url) {
        return Rx.Observable.fromPromise(axios.get(url)).map(function (resGit) {
            return resGit.data;
        });
    });

    reponseStream.subscribe(function (resGit) {
        res.send(resGit);
    }, function (error) {
        res.send({ status: 'error' });
    }, function () {
